...
{
    public abstract class HttpCustomerTriggerBase<T>: HttpTriggerBase
    {
        protected T Request;

        protected override HttpResponseMessage SafeInternalHandle()
        {
            TrackCustomer(Request?.CustomerId);
            return base.SafeInternalHandle();
        }

        protected void TrackJob(string jobId)
        {
            Log.TrackExecutionInfo(Request?.CustomerId, jobId);
        }
    }
}
